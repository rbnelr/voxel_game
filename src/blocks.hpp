...
struct Block_Texture_Source {
	...
	cstr	texture_filename;
	cstr	alpha_texture_filename;
};
...
static s32 atlas_textures_count = ARRLEN(block_texture_sources);
...
static Texture2D* generate_and_upload_block_texture_atlas () { // texture atlasing
	...
	dbg_assert(tex->get_pixel_size() == 4);
	u32* src_pixels;
	u32* aplha_src_pixels;
	u32* dst_pixels = (u32*)tex->data.data;
	...
	auto aplha_src = [&] (s32 x, s32 y, s32 face) -> u32* {
		s32 w = texture_res;
		s32 h = texture_res;
	...
	for (s32 tex_index=0; tex_index<atlas_textures_count; ++tex_index) {
		...
		col_tex.load();
		dbg_assert(col_tex.type == PT_SRGB8_LA8);
		dbg_assert(all(col_tex.dim == iv2(texture_res, texture_res*ATLAS_BLOCK_FACES_COUNT)));
		dbg_assert(col_tex.get_pixel_size() == 4);
		
		src_pixels = (u32*)col_tex.data.data;
		
		if (src_tex.optional_apha) {
			alpha_tex.load();
			dbg_assert(alpha_tex.type == PT_SRGB8_LA8);
			dbg_assert(all(alpha_tex.dim == iv2(texture_res, texture_res*ATLAS_BLOCK_FACES_COUNT)));
			dbg_assert(alpha_tex.get_pixel_size() == 4);
			
			aplha_src_pixels = (u32*)alpha_tex.data.data;
		}
		...
		for (s32 block_face_i=0; block_face_i<ATLAS_BLOCK_FACES_COUNT; ++block_face_i) {
			...
			for (s32 y=0; y<texture_res; ++y) {
				...
				for (s32 x=0; x<texture_res; ++x) {
					...
					if (src_tex.optional_apha) {
						u32 alpha = *aplha_src(x,y, block_face_i);
						col &= ~0xff000000;
						col |= (alpha & 0xff) << 24;
					}

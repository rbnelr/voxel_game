...
void VCT_Data::recompute_mips (OpenglRenderer& r, std::vector<int3> const& chunks) {
	...
	//// I know this code looks horrible but the setup of textures and how to preintegrate (filter) them is pretty complex
	// step1: generate full resolution data from voxel data
	// step2: generate first mip (half resolution) which is actually a seperate set of textures
	//         from full resolution texture (thus it's mip0 rather than mip1)
	// step3: generate remaining mips from each previous mip

	static constexpr int BATCHSIZE = 16;
	int3 offsets[BATCHSIZE];

	auto dispatch_chunk_area = [&] (Shader* shad, int mip, int first_chunk_i, int num_chunks) {
		int size = CHUNK_SIZE >> mip;

		memset(offsets, 0, sizeof(offsets));
		for (int j=0; j<num_chunks; ++j)
			offsets[j] = (chunks[first_chunk_i+j] * CHUNK_SIZE) >> mip;
		glUniform3uiv(shad->get_uniform_location("offsets[0]"), ARRLEN(offsets), (GLuint*)offsets);
		shad->set_uniform("read_mip", mip-1-1); // -1 for previous mip to read  another  -1 for first mip being mip 1
		shad->set_uniform("size", (GLuint)size);

		int dispatch_size = (size + COMPUTE_FILTER_LOCAL_SIZE-1) / COMPUTE_FILTER_LOCAL_SIZE;
		glDispatchCompute(dispatch_size, dispatch_size, dispatch_size * num_chunks);
	};

	{ // layer 0, generate texture from voxel data in voxel textures

		glUseProgram(filter_base->prog);
		r.state.bind_textures(filter_base, {
			{"subchunks_tex", r.raytracer.subchunks_tex.tex},
			{"voxels_tex", r.raytracer.voxels_tex.tex},
			{"tile_textures", r.tile_textures, r.tile_sampler},
		});

		glBindImageTexture(0, basedata, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);

		for (int i=0; i<(int)chunks.size(); i+=BATCHSIZE) {
			int remain_count = min(BATCHSIZE, (int)chunks.size() - i);
			dispatch_chunk_area(filter_base, 0, i, remain_count);
		}

		glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT|GL_TEXTURE_FETCH_BARRIER_BIT);
	}

	{ // first mip -> previous data is basedata
		glUseProgram(filter_preint0->prog);

		r.state.bind_textures(filter_preint0, {
			{"vct_basedata", basedata, filter_sampler},
		});

		// filter only texels for each chunk (up to 4x4x4 work groups)
		for (int mip=1; mip<FILTER_CHUNK_MIPS; ++mip) {
			for (int dir=0; dir<6; ++dir)
				glBindImageTexture(dir, preint[dir], mip-1, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);

			for (int i=0; i<(int)chunks.size(); i+=BATCHSIZE) {
				int remain_count = min(BATCHSIZE, (int)chunks.size() - i);
				dispatch_chunk_area(filter_preint0, mip, i, remain_count);
			}
	...
	{ // remaing mips -> previous data is previous mip
		glUseProgram(filter_preint1->prog);

		r.state.bind_textures(filter_preint1, {
			{"vct_basedata", basedata, filter_sampler},
			{"vct_preintNX", preint[0], filter_sampler},
			{"vct_preintNX", preint[0], filter_sampler},
			{"vct_preintPX", preint[1], filter_sampler},
			{"vct_preintNY", preint[2], filter_sampler},
			{"vct_preintPY", preint[3], filter_sampler},
			{"vct_preintNZ", preint[4], filter_sampler},
			{"vct_preintPZ", preint[5], filter_sampler},
		});

		// filter only texels for each chunk (up to 4x4x4 work groups)
		for (int mip=2; mip<FILTER_CHUNK_MIPS; ++mip) {
			for (int dir=0; dir<6; ++dir)
				glBindImageTexture(dir, preint[dir], mip-1, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);

			for (int i=0; i<(int)chunks.size(); i+=BATCHSIZE) {
				int remain_count = min(BATCHSIZE, (int)chunks.size() - i);
				dispatch_chunk_area(filter_preint1, mip, i, remain_count);
			}
		...
		memset(offsets, 0, sizeof(offsets));
		glUniform3uiv(filter_preint1->get_uniform_location("offsets[0]"), ARRLEN(offsets), (GLuint*)offsets);

		for (int layer=FILTER_CHUNK_MIPS; layer<MIPS; ++layer) {
			int size = TEX_WIDTH >> layer;
			filter_preint1->set_uniform("read_mip", layer-2);
			filter_preint1->set_uniform("size", (GLuint)size);

			for (int dir=0; dir<6; ++dir)
				glBindImageTexture(dir, preint[dir], layer, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);
	...
	// unbind
	for (int dir=0; dir<6; ++dir)
		glBindImageTexture(dir, 0, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);
}
...
void Raytracer::draw (OpenglRenderer& r, Game& game) {
	...
	r.state.bind_textures(shad, {
		...
		{"vct_basedata", vct_data.basedata,  vct_data.filter_sampler},
		{"vct_preintNX", vct_data.preint[0], vct_data.filter_sampler},
		{"vct_preintNX", vct_data.preint[0], vct_data.filter_sampler},
		{"vct_preintPX", vct_data.preint[1], vct_data.filter_sampler},
		{"vct_preintNY", vct_data.preint[2], vct_data.filter_sampler},
		{"vct_preintPY", vct_data.preint[3], vct_data.filter_sampler},
		{"vct_preintNZ", vct_data.preint[4], vct_data.filter_sampler},
		{"vct_preintPZ", vct_data.preint[5], vct_data.filter_sampler},

...
void RadianceCascadesTesting::update (Game& game, OpenglRenderer& r) {
	...
	if (trace_shad->prog) {
		...
		for (int casc=cascades-1; casc>=0; casc--) {
			...
			int2 ray_regions = get_ray_regions(casc);
		
			trace_shad->set_uniform("higher_cascade_spacing", has_higher_cascade ? get_spacing(casc+1) : 0.0f);
			...
			trace_shad->set_uniform("ray_regions", ray_regions);
			...
			int2 dispatch_size = (tex.size + GROUP_SZ-1) / GROUP_SZ;
	...
	if (combine_shad->prog) {
		...
		int2 num_probes = get_num_probes(spacing);
		int2 ray_regions = get_ray_regions(0);

		combine_shad->set_uniform("dispatch_size", result_tex.size);
		combine_shad->set_uniform("num_probes", num_probes);
		combine_shad->set_uniform("ray_regions", ray_regions);
		
		glBindImageTexture(4, result_tex.tex, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA16F);

		int2 dispatch_size = (result_tex.size + GROUP_SZ-1) / GROUP_SZ;
		glDispatchCompute(dispatch_size.x, dispatch_size.y, 1);
	}
	
	auto show = [&] () {
		auto& tex = show_cascade >= 0 ? cascade_texs[show_cascade].tex : result_tex.tex;

		float tex_spacing = get_spacing(max(show_cascade, 0));
		float2 tex_size = (float2)get_num_probes(tex_spacing) * tex_spacing;
		tex_draw.draw(r.state, (float3)base_pos + float3(0,1,0), float3((float2)tex_size, 1), tex, true);

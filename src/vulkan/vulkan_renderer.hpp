...
struct Renderer {
	...
	void frame_begin (GLFWwindow* window) {
		...
		while (true) {
			...
			if (res == VK_ERROR_OUT_OF_DATE_KHR || res == VK_SUBOPTIMAL_KHR) {
				...
				// don't break; acuire new swap chain image, should work this time
			} else {
				VK_CHECK_RESULT(res);
				break;
			}
	...
	void set_view_uniforms (Camera_View& view, int2 viewport_size) {
		...
		void* data;
		vkMapMemory(device, ubo_memory, 0, sizeof(ViewUniforms), 0, &data);
	...
	void render_frame (GLFWwindow* window, RenderData& data) {
		...
		{
			vkCmdBindDescriptorSets(buf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, 1,
	...
	Renderer (char const* app_name, GLFWwindow* window) {
		...
		imgui_renderer_create(window, msaa);
	}
	~Renderer () {
		ZoneScoped;
	...
	void create_descriptor_pool () {
		...
		VK_CHECK_RESULT(vkCreateDescriptorPool(device, &info, nullptr, &descriptor_pool));
	}
	...
	void create_descriptors () {
		{ // create descriptor layout
			...
			VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device, &info, nullptr, &descriptor_layout));
		}
		...
		{ // create descriptor sets
			...
			VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &info, sets));
	...
	void create_ubo_buffers () {
		...
		for (auto& f : frame_data) {
			...
			VK_CHECK_RESULT(vkCreateBuffer(device, &info, nullptr, &f.ubo_buffer));

			VkMemoryRequirements mem_req;
			vkGetBufferMemoryRequirements(device, f.ubo_buffer, &mem_req);
		...
		{ // alloc gpu-resident host-visible ubo memory
			...
			info.memoryTypeIndex = find_memory_type(physical_device, mem_req_bits,
				VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
			VK_CHECK_RESULT(vkAllocateMemory(device, &info, nullptr, &ubo_memory));

			for (auto& f : frame_data)
				vkBindBufferMemory(device, f.ubo_buffer, ubo_memory, f.ubo_mem_offset);
		}
	}
	void destroy_ubo_buffers () {
		for (auto& f : frame_data)
			vkDestroyBuffer(device, f.ubo_buffer, nullptr);
	...
	//// Imgui
	void imgui_renderer_create (GLFWwindow* window, int msaa) {
		ZoneScoped;
		...
		info.DescriptorPool		= descriptor_pool;
		info.MinImageCount		= SWAP_CHAIN_SIZE;
		info.ImageCount			= SWAP_CHAIN_SIZE;
		info.MSAASamples		= (VkSampleCountFlagBits)msaa;
		info.Allocator			= nullptr;
		info.CheckVkResultFn	= nullptr;
		ImGui_ImplVulkan_Init(&info, render_pass);

		auto cmd_buf = begin_one_time_commands();

		ImGui_ImplVulkan_CreateFontsTexture(cmd_buf);

		end_one_time_commands(cmd_buf);
	}
	void imgui_renderer_destroy () {
		ZoneScoped;

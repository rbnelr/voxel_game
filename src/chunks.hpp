...
struct Chunk {
	...
	void remesh () {
		...
		auto transperant_cube = [&] (bpos const& pos_world, bpos const& pos_chunk, Block const* b) {
			...
			bt = query_block(pos_world +bpos(+1,0,0))->type;
			if (bt_is_transparent(bt) && bt != b->type) {
				Chunk_Vbo_Vertex* out = (Chunk_Vbo_Vertex*)&*vector_append(&vbo_transperant.vertecies, sizeof(Chunk_Vbo_Vertex)*6);
			...
			bt = query_block(pos_world +bpos(0,+1,0))->type;
			if (bt_is_transparent(bt) && bt != b->type) {
				Chunk_Vbo_Vertex* out = (Chunk_Vbo_Vertex*)&*vector_append(&vbo_transperant.vertecies, sizeof(Chunk_Vbo_Vertex)*6);
			...
			bt = query_block(pos_world +bpos(0,0,+1))->type;
			if (bt_is_transparent(bt) && bt != b->type) {
				Chunk_Vbo_Vertex* out = (Chunk_Vbo_Vertex*)&*vector_append(&vbo_transperant.vertecies, sizeof(Chunk_Vbo_Vertex)*6);
				...
				for (i.y=0; i.y<CHUNK_DIM.y; ++i.y) {
					for (i.x=0; i.x<CHUNK_DIM.x; ++i.x) {
						...
						if (block->type != BT_AIR) {
							...
							if (bt_is_transparent(block->type))	transperant_cube(i +chunk_origin, i, block);
							else								cube(i +chunk_origin, i, block);

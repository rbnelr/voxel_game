...
struct Timing_Histogram {
	...
	void imgui (char const* name, float seconds, float dt, bool default_open=false) { // dt for updating imgui every update_period (not for actual value being averaged)
		avg.push(seconds);
		...
		if (ImGui::TreeNodeEx(name, default_open ? ImGuiTreeNodeFlags_DefaultOpen : 0,
			...
			ImGui::PlotHistogram("##frametimes_histogram", avg.data(), (int)avg.count(), 0, "frametimes:", 0, 1.0f/20, ImVec2(0, (float)histogram_height));

...
void Game::frame () {
	...
	if (activate_flycam) {
		...
		view = world->player.update_post_physics();
	}

	Block*	highlighted_block = nullptr;
	bpos	highlighted_block_pos;
	int		highlighted_block_face = -1;
	...
	//
	//	highlighted_block = world.raycast_block(ray_pos, ray_dir, 5.0f, &highlighted_block_pos, &highlighted_block_face);
	//};
	...
	if (highlighted_block && input.buttons[GLFW_MOUSE_BUTTON_LEFT].is_down) { // block breaking
		...
		Chunk* chunk;
		Block* b = world->chunks.query_block(highlighted_block_pos, &chunk);
		...
		if (b->hp_ratio > 0) {
			...
			chunk->block_changed(world->chunks, highlighted_block_pos);
	...
	{ // block placing
		...
		if (trigger_place_block && highlighted_block && highlighted_block_face >= 0) {

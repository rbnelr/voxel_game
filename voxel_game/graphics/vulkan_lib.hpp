...
namespace vk {
	...
	std::vector<T> inline get_vector (FUNC func, ARGS... args) {
		uint32_t count = 0;
	...
	inline VkBuffer create_buffer_and_memory (VkDevice device, VkPhysicalDevice pdevice, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags props, VkDeviceMemory* out_memory) {
		...
		VkResult res = vkCreateBuffer(device, &info, nullptr, &buffer);
		...
		alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
		alloc_info.allocationSize = mem_req.size;
		alloc_info.memoryTypeIndex = find_memory_type(pdevice, mem_req.memoryTypeBits, props);
	...
	inline VkInstance create_instance (char const* app_name, PFN_vkDebugUtilsMessengerCallbackEXT callback, std::vector<char const*>* out_layers) {

...
static void APIENTRY ogl_debuproc (GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const char* message, void const* userParam) {
	...
	if (severity == GL_DEBUG_SEVERITY_HIGH_ARB && 0) {
		fprintf(stderr, false, "OpenGL debug proc: severity: %s src: %s type: %s id: %d  %s\n",
...
static void platform_setup_context_and_open_window (const char* inital_wnd_title, int2 default_wnd_dim) {
	...
	{ // open and postion window
		...
		wnd = glfwCreateWindow(1280,720, inital_wnd_title, NULL, NULL);
		assert(wnd);
	}

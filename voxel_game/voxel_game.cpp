...
#define PROFILE_BEGIN(name)	double __profile_begin_##name = glfwGetTime()
...
static const char* shaders_base_path =		"shaders/";
...
static Shader* new_shader (std::string const& v, std::string const& f, std::initializer_list<Uniform> u, std::initializer_list<Shader::Uniform_Texture> t={}) {
...
static float elev_freq = 400, elev_amp = 25;
static float rough_freq = 220;
static float detail0_freq = 70, detail0_amp = 12;
static float detail1_freq = 20, detail1_amp = 3;
static float detail2_freq = 3, detail2_amp = 0.14f;
...
static float noise_tree_desity_period = 200;
static float noise_tree_density_amp = 1;

static float noise_tree_density (OSN::Noise<2> const& osn_noise, float2 pos_world) {
...
static void inital_chunk (float3 player_pos_world) {
...
struct Input {
	int2		wnd_dim;
	float2		wnd_dim_aspect;
	
	int2		mcursor_pos_px;
	
	float2		mouse_look_diff;
	float		mouse_wheel_diff;
	...
	void get_non_callback_input () {
		{
			...
			float2 tmp = (float2)wnd_dim;
			wnd_dim_aspect = tmp / float2(tmp.y, tmp.x);
		}
		{
			double x, y;
			glfwGetCursorPos(wnd, &x, &y);
			int2 tmp = int2((int)x, (int)y);
			
			if (mouselook_enabled) mouse_look_diff = (float2)(tmp -mcursor_pos_px);
			
			mcursor_pos_px = tmp;
			
			mouse = float2((float)x,(float)y) / (float2)wnd_dim;
		}
		
		move_dir = clamp(move_dir, -1,+1);
	}
	
	float2 bottom_up_mcursor_pos () {
		return float2((float)mcursor_pos_px.x, (float)(wnd_dim.y -mcursor_pos_px.y));
	}
};
static Input		inp;
...
static void glfw_mouse_scroll (GLFWwindow* window, double xoffset, double yoffset) {
	inp.mouse_wheel_diff += (float)yoffset;
...
int main (int argc, char** argv) {
	...
	const char* app_name = "Voxel Game";
	
	platform_setup_context_and_open_window(app_name, int2(1280, 720));
	...
	float avg_dt = 1.0f / 60;
	float avg_dt_alpha = 0.025f;
	dt = 0;
	
	bool fixed_dt = 1 || false/*IS_DEBUGGER_PRESENT()*/; 
	float max_variable_dt = 1.0f / 20; 
	float fixed_dt_dt = 1.0f / 60; 
	...
	for (frame_i=0;; ++frame_i) {
		...
		auto overlay_line = [] (std::string const& str, float3 col=0) {};
		...
		{ // player position (collision and movement dynamics)
			...
			if (player_stuck_in_solid_block) {
				...
				vel_world += float3(0,0, -grav_accel_down) * dt;
			}
		...
		if (1) { // chunk update
			...
			bpos_t blocks_to_update = (bpos_t)ceil((float)chunk_block_count * block_update_frequency * dt);
		...
		for (auto* s : shaders) { // set common uniforms
			if (s->valid()) {
				s->bind();
				s->set_unif("screen_dim", (float2)inp.wnd_dim);

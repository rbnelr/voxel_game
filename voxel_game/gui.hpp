...
static ImVec2 imgui_v2 (int2 v) {	return ImVec2((float)v.x, (float)v.y); }
static ImVec2 imgui_v2 (float2 v) {		return ImVec2(v.x,v.y); }
...
static void imgui_init () {
	...
	imgui_vbo.init();
	{
		...
		font_atlas->alloc_cpu_single_mip(PT_SRGB8_LA8, int2(w,h));
...
struct Imgui_Texture_Window {
	...
	void imgui () {
		...
		if (tex) {
			float2 size = ImGui::GetContentRegionAvailWidth();
...
static void imgui_begin (float dt, int2 wnd_dim, int2 mcursor_pos_px, float mouse_wheel_diff, bool lmb_down, bool rmb_down) {
	ImGuiIO& io = ImGui::GetIO();
	
	io.DisplaySize.x = (float)wnd_dim.x;
	io.DisplaySize.y = (float)wnd_dim.y;
	io.DeltaTime = dt;
	io.MousePos.x = (float)mcursor_pos_px.x;
	io.MousePos.y = (float)mcursor_pos_px.y;
	...
	// texture_windows
	for (int i=0; i<(int)imgui_texture_windows.size(); ++i) {
...
static void draw_imgui (int2 wnd_dim) {
	ImGui::End();
	ImGui::Render();
	...
	shad_imgui->bind();
	shad_imgui->set_unif("screen_dim", (float2)wnd_dim);
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++) {
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback) {
				...
				float y0 = (float)wnd_dim.y -pcmd->ClipRect.w;
				float y1 = (float)wnd_dim.y -pcmd->ClipRect.y;
				
				
				
				glScissor((int)pcmd->ClipRect.x, (GLint)y0, (int)(pcmd->ClipRect.z -pcmd->ClipRect.x), (int)(y1 -y0));
				...
				glDrawElements(GL_TRIANGLES, pcmd->ElemCount, GL_UNSIGNED_SHORT,
					(GLvoid const*)((uint8_t const*)cur_idx_buffer -(uint8_t const*)idx_buffer));
			}

...
layout(location = 0) vs2fs VS {
	...
	void main () {
		//vec3 col = texture(main_albedo, vs.uv).rgb;
		//for (int i=0; i<4; ++i)
		//	col += texture(main_albedo, vs.uv + blur_kernel[i]*vec2(0.01)).rgb;
		//frag_col = vec4(col / vec3(5), 1.0);

		vec3 normal = vec3(0,0,+1);
		//normal.xy = texture(main_norm, uv).rg;
		...
		for (int i=0; i<count; ++i) {
			vec3 s = (TBN * rand_samples[i]) * radius;
			//vec3 s = rand_samples[i] * radius;
			float d = get_depth(vs.uv + s.xy);
			
			if (depth + s.z <= d)
				non_occluded++;
		}
		
		float fac = non_occluded / float(count);
